...
( ( root, cx ) => {
	...
	const albumArt = async ( artist, options, cb ) => {
		...
		const queryParams = `?q=${encodeURIComponent( query )}&type=track&market=KR&limit=1`
		...
		const response = !error && await fetch( searchUrl, {
            ...
            .then(
                json => {
                    ...
                    // Select image size
                    const images = json.tracks.items[0].album.images; // Access the images through the album object
